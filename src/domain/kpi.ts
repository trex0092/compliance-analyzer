...
export function calculateKPI(
  ...
  // ─── Cases ─────────────────────────────────────────────────────────────
  const openCasesList = cases.filter(
  ...
  const caseAges = openCasesList.map(
    (c) => (now - new Date(c.createdAt).getTime()) / 86400000
  );
  const avgCaseAgeDays = caseAges.length > 0
    ? Math.round(caseAges.reduce((a, b) => a + b, 0) / caseAges.length)
    : 0;
  ...
  const cddReviewOnTimePct = reviewAlerts.length > 0
  ...
  const strFilingTimelinessPct = strFiled.length > 0
    ? Math.round((strOnTime.length / strFiled.length) * 100)
    : 100;
  ...
  const totalPotentialMatches = screenings.filter(
    (s) => s.result === "potential-match"
  ).length;
  const screeningFalsePositiveRate = totalPotentialMatches > 0
  ...
  const approvalTurnarounds = decidedApprovals.map((a) =>
    (new Date(a.decidedAt!).getTime() - new Date(a.requestedAt).getTime()) / 3600000
  );
  const avgApprovalTurnaroundHours = approvalTurnarounds.length > 0
  ...
  const blockedShipments = cases.filter(
    (c) =>
      ...
      c.redFlags.some(
        (f) => f === "RF011" || f === "RF012" || f === "RF041" || f === "RF070"
      ) &&
      c.status !== "closed"
  ).length;
