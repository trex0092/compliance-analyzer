...
export function KPIDashboardView({ data }: KPIDashboardProps) {
  const overallStatus = useMemo(() => {
    if (data.criticalCases > 0 || data.criticalAlerts > 0 || data.auditReadinessPct < 50) return "critical";
    if (data.overdueReviews > 0 || data.strPending > 0 || data.auditReadinessPct < 80) return "warning";
    return "good";
  }, [data]);
  ...
  return (
    <div style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: "12px", color: "#888", marginBottom: "8px" }}>MOE AUDIT READINESS</div>
        <div style={{ display: "flex", alignItems: "baseline", gap: "8px" }}>
          <span style={{ fontSize: "36px", fontWeight: 700, color: data.auditReadinessPct >= 80 ? "#3DA876" : data.auditReadinessPct >= 50 ? "#E8A030" : "#D94F4F" }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            height: "100%",
            background: data.auditReadinessPct >= 80 ? "#3DA876" : data.auditReadinessPct >= 50 ? "#E8A030" : "#D94F4F",
            borderRadius: "3px",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: "10px", marginBottom: "16px" }}>
        ...
        <MetricCard label="STR Filing Rate" value={data.strFilingTimelinessPct} target={90} unit="%" />
        <MetricCard label="Evidence Complete" value={data.evidenceCompletionPct} target={80} unit="%" />
      ...
      {/* Filing Summary */}
      <div style={{ padding: "12px 16px", background: "#0f0f23", borderRadius: "6px", border: "1px solid #2a2a4a" }}>
        ...
        <div style={{ display: "flex", gap: "24px", fontSize: "13px", color: "#e0e0e0" }}>
          <span>STR: <strong>{data.strCount}</strong></span>
          <span>SAR: <strong>{data.sarCount}</strong></span>
          <span>CTR: <strong>{data.ctrCount}</strong></span>
          <span>Blocked Shipments: <strong>{data.blockedShipments}</strong></span>
        </div>
      </div>
    </div>
  );
