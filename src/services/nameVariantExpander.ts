...
const MANDARIN_PINYIN: Readonly<Record<string, string>> = {
  ...
  '刚': 'gang', '桂': 'gui', '华': 'hua', '玲': 'ling', '飞': 'fei',
  '波': 'bo', '宁': 'ning', '国': 'guo', '雪': 'xue', '冰': 'bing',
  '红': 'hong', '燕': 'yan', '辉': 'hui', '健': 'jian', '建': 'jian',
};
...
const JAPANESE_SURNAMES: Readonly<Record<string, string>> = {
  '田中': 'tanaka', '鈴木': 'suzuki', '佐藤': 'sato', '高橋': 'takahashi',
  ...
  '木村': 'kimura', '林': 'hayashi', '清水': 'shimizu', '山崎': 'yamazaki',
...
function romaniseCjk(raw: string): string[] {
  ...
  const firstRoman =
    MANDARIN_PINYIN[firstChar] ??
    KOREAN_ROMANISATION[firstChar] ??
    null;
