...
export function extractAmounts(text: string): ExtractedAmount[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const currPart = match[0]
        .replace(numStr, '')
        .replace(/[,.\s]/g, '')
        .toLowerCase();
      const currency =
        Object.entries(currencyMap).find(([k]) => currPart.includes(k))?.[1] ?? 'AED';
      ...
      results.push({
        ...
        threshold:
          currency === 'AED' && value >= DPMS_CASH_THRESHOLD_AED
            ? `Exceeds AED ${DPMS_CASH_THRESHOLD_AED.toLocaleString()} DPMS threshold`
            : undefined,
      });
    }
  }
...
export function extractDates(text: string): ExtractedDate[] {
  const patterns = [
    ...
    {
      regex: /(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\s+(\d{4})/gi,
      format: 'dMy',
    },
  ];
  ...
  for (const { regex, format } of patterns) {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      if (format === 'dmy') {
        ...
        day = parseInt(match[1]);
        month = monthMap[match[2].toLowerCase().slice(0, 3)] ?? 1;
        year = parseInt(match[3]);
      }
...
export function extractEntities(text: string): ExtractedEntity[] {
  ...
  const companyRegex =
    /(?:[A-Z][\w]*(?:\s+[A-Z][\w]*)*)\s+(?:LLC|Ltd|Limited|DMCC|FZE|FZC|FZCO|Inc|Corp|Trading|International)\b/g;
  ...
  for (const country of countries) {
    ...
    while (idx !== -1) {
      entities.push({
        ...
        type: 'location',
        confidence: 0.9,
        position: { start: idx, end: idx + country.length },
      });
...
export function detectDocumentRedFlags(text: string): DocumentRedFlag[] {
  ...
  const redFlagPatterns: Array<{
    pattern: RegExp;
    type: string;
    severity: DocumentRedFlag['severity'];
    description: string;
    regulatoryRef: string;
  }> = [
    {
      pattern: /no\s+(?:source|proof|evidence)\s+of\s+(?:funds|wealth|income)/i,
      type: 'missing-sof',
      severity: 'high',
      description: 'Missing source of funds/wealth documentation',
      regulatoryRef: 'FDL Art.12-14',
    },
    {
      pattern: /cash\s+(?:payment|deposit|transaction)\s+(?:of\s+)?(?:AED|Dhs?)?\s*[\d,]+/i,
      type: 'large-cash',
      severity: 'high',
      description: 'Cash transaction mentioned — verify against thresholds',
      regulatoryRef: 'FDL Art.15-16',
    },
    {
      pattern: /(?:bearer|anonymous|unnamed|unidentified)\s+(?:instrument|check|cheque|bond)/i,
      type: 'bearer-instrument',
      severity: 'critical',
      description: 'Bearer/anonymous instrument detected',
      regulatoryRef: 'FDL Art.15, FATF Rec 22',
    },
    {
      pattern: /(?:third\s*party|nominee|agent)\s+(?:payment|transfer|deposit)/i,
      type: 'third-party',
      severity: 'high',
      description: 'Third-party payment arrangement',
      regulatoryRef: 'Cabinet Res 134/2025 Art.9',
    },
    {
      pattern: /(?:urgently?|rush|immediate|asap)\s+(?:transfer|process|clear|approve)/i,
      type: 'urgency-pressure',
      severity: 'medium',
      description: 'Unusual urgency/pressure language',
      regulatoryRef: 'FATF ML Typologies',
    },
    {
      pattern: /(?:no\s+questions?|don't\s+ask|confidential|off[\s-]*(?:the[\s-]*)?record)/i,
      type: 'secrecy',
      severity: 'critical',
      description: 'Secrecy/anti-transparency language detected',
      regulatoryRef: 'FDL Art.29, FATF Rec 22',
    },
    {
      pattern:
        /(?:split|divide|break\s+(?:up|down)|separate)\s+(?:the\s+)?(?:payment|amount|transaction)/i,
      type: 'structuring-language',
      severity: 'critical',
      description: 'Structuring language detected — possible threshold avoidance',
      regulatoryRef: 'FDL Art.15-16',
    },
    {
      pattern: /(?:free[\s-]*(?:trade)?[\s-]*zone|FTZ|offshore|shell\s+company)/i,
      type: 'opacity-indicator',
      severity: 'medium',
      description: 'Opacity/offshore indicator in document',
...
export function analyzeDocument(
  ...
  if (!documentType) {
    ...
    else if (/passport|emirates\s+id|nationality|date\s+of\s+birth/i.test(text))
      detectedType = 'kyc';
    else if (/bill\s+of\s+lading|certificate\s+of\s+origin|hs\s+code|customs/i.test(text))
      detectedType = 'trade';
  ...
  if (dualUseKeywords.length > 0)
    riskIndicators.push(`${dualUseKeywords.length} dual-use keyword(s)`);
  if (entities.filter((e) => e.type === 'company').length >= 3)
    riskIndicators.push('Multiple companies referenced');
...
export const DOCUMENT_TOOL_SCHEMAS = [
  {
    ...
    description:
      'Extract compliance data from documents using NLP: amounts, entities, dates, red flags, dual-use keywords. Auto-detects document type (invoice/KYC/trade/narrative). Returns completeness score.',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string' },
        documentType: { type: 'string', enum: ['invoice', 'kyc', 'trade', 'narrative'] },
      },
      required: ['text'],
    },
  },
