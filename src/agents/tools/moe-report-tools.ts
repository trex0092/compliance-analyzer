...
export function runReadinessCheck(
  ...
  // 1. AML/CFT Policy
  addItem(1, 'AML/CFT Policy & Procedures', 'Governance', 4, true,
    ds.riskAssessmentCurrent, false,
    ds.riskAssessmentCurrent ? 'AML/CFT policy and risk assessment are current.' : 'Risk assessment is outdated. Policy review required.',
    ds.riskAssessmentCurrent ? undefined : 'Update enterprise risk assessment and AML/CFT policies within 30 days.');

  // 2. CO Appointment
  addItem(2, 'Compliance Officer Appointment', 'Governance', 4, true,
    ds.complianceOfficerAppointed, false,
    ds.complianceOfficerAppointed ? 'Compliance Officer is appointed.' : 'No Compliance Officer appointed.',
    ds.complianceOfficerAppointed ? undefined : 'Appoint a CO immediately and notify MoE per Cabinet Res 134/2025 Art.18.');

  // 3. CO Notification
  addItem(3, 'CO Notification to MoE', 'Governance', 3, false,
    ds.coNotifiedToMoE, false,
    ds.coNotifiedToMoE ? 'CO notification to MoE confirmed.' : 'CO notification to MoE not recorded.',
    ds.coNotifiedToMoE ? undefined : 'Submit CO notification to MoE within 5 days.');

  // 4. Risk Assessment
  addItem(4, 'Risk Assessment (Enterprise-wide)', 'Risk', 4, true,
    ds.riskAssessmentCurrent, false,
    ds.riskAssessmentCurrent ? 'Enterprise risk assessment is current.' : 'Enterprise risk assessment is outdated or missing.',
  ...
  addItem(5, 'Customer Risk Rating Methodology', 'Risk', 3, false,
    cddRate >= 0.95, cddRate >= 0.7,
    `CDD completion rate: ${(cddRate * 100).toFixed(1)}% (${ds.cddCompletedCount}/${ds.totalActiveCustomers}).`,
    cddRate >= 0.95 ? undefined : `Complete CDD for remaining ${ds.totalActiveCustomers - ds.cddCompletedCount} customers.`);

  // 6. CDD/KYC Procedures
  addItem(6, 'CDD / KYC Procedures', 'CDD', 4, true,
    cddRate >= 0.95 && ds.cddOverdueRenewals === 0, cddRate >= 0.7,
    `CDD rate: ${(cddRate * 100).toFixed(1)}%. Overdue renewals: ${ds.cddOverdueRenewals}.`,
    ds.cddOverdueRenewals > 0 ? `Address ${ds.cddOverdueRenewals} overdue CDD renewals immediately.` : undefined);

  // 7. EDD for High-Risk
  addItem(7, 'EDD for High-Risk Customers', 'CDD', 4, true,
    ds.eddAppliedToHighRisk, false,
    ds.eddAppliedToHighRisk ? 'EDD is applied to all high-risk customers.' : 'EDD not consistently applied to high-risk customers.',
    ds.eddAppliedToHighRisk ? undefined : 'Implement EDD for all high-risk customers per Cabinet Res 134/2025 Art.14.');

  // 8. PEP Identification
  addItem(8, 'PEP Identification & EDD', 'CDD', 4, true,
    ds.pepScreeningActive, false,
  ...
  // 9. UBO
  addItem(9, 'Beneficial Ownership (UBO)', 'CDD', 3, true,
    ds.uboRegisterCurrent, false,
    ds.uboRegisterCurrent ? 'UBO register is current.' : 'UBO register is not current.',
    ds.uboRegisterCurrent ? undefined : 'Update UBO register per Cabinet Decision 109/2023. Re-verify within 15 working days.');

  // 10. Ongoing Monitoring
  addItem(10, 'Ongoing Monitoring', 'Monitoring', 4, true,
    ds.screeningsPerformed > 0 && cddRate >= 0.9, ds.screeningsPerformed > 0,
  ...
  addItem(11, 'Transaction Monitoring System', 'Monitoring', 4, true,
    ds.ctrsFiledCount > 0 || ds.strsFiledCount > 0, true,
    `STRs filed: ${ds.strsFiledCount}. CTRs filed: ${ds.ctrsFiledCount}. System operational.`,
    undefined);

  // 12. Sanctions Screening
  addItem(12, 'Sanctions Screening (all lists)', 'Screening', 4, true,
    ds.allSanctionsListsActive, false,
    ds.allSanctionsListsActive ? 'All sanctions lists (UN, OFAC, EU, UK, UAE, EOCN) are active.' : 'Not all sanctions lists are active.',
    ds.allSanctionsListsActive ? undefined : 'CRITICAL: Activate ALL sanctions lists per FDL Art.35. Never skip a list.');

  // 13. TFS/Freeze Capability
  addItem(13, 'TFS / Asset Freeze Capability', 'Screening', 4, true,
    ds.tfsFreezeCapabilityActive, false,
    ds.tfsFreezeCapabilityActive ? 'TFS freeze capability is operational (24h execution ready).' : 'TFS freeze capability NOT operational.',
    ds.tfsFreezeCapabilityActive ? undefined : 'CRITICAL: Implement asset freeze capability per Cabinet Res 74/2020. Must execute within 24 hours.');

  // 14. goAML Registration
  addItem(14, 'goAML Registration & Access', 'Filing', 3, true,
    ds.goAMLRegistered, false,
    ds.goAMLRegistered ? 'goAML registration confirmed and accessible.' : 'goAML not registered.',
    ds.goAMLRegistered ? undefined : 'Register with goAML per MoE Circular 08/AML/2021 within 10 business days.');

  // 15. STR Filing Procedures
  const strOnTimeRate = ds.strsFiledCount > 0 ? ds.strsFiledOnTime / ds.strsFiledCount : 1;
  addItem(15, 'STR/SAR Filing Procedures', 'Filing', 4, true,
    strOnTimeRate >= 1 && ds.goAMLRegistered, strOnTimeRate >= 0.8,
  ...
  addItem(16, 'CTR / DPMSR Filing (AED 55K)', 'Filing', 3, false,
    ctrOnTimeRate >= 1, ctrOnTimeRate >= 0.8,
  ...
  // 17. Record Retention
  addItem(17, 'Record Retention (5 years min)', 'Records', 3, false,
    ds.recordRetentionCompliant, false,
    ds.recordRetentionCompliant ? 'Record retention policy meets minimum requirements.' : 'Record retention policy deficient.',
    ds.recordRetentionCompliant ? undefined : 'Implement record retention meeting 5-year minimum per FDL Art.24.');

  // 18. Staff Training
  const trainingRate = ds.totalStaffRequiringTraining > 0 ? ds.staffTrained / ds.totalStaffRequiringTraining : 0;
  addItem(18, 'Staff Training Programme', 'Training', 3, false,
    trainingRate >= 0.95, trainingRate >= 0.7,
    `Training completion: ${(trainingRate * 100).toFixed(0)}% (${ds.staffTrained}/${ds.totalStaffRequiringTraining}).`,
    trainingRate < 0.95 ? `Complete training for remaining ${ds.totalStaffRequiringTraining - ds.staffTrained} staff members.` : undefined);

  // 19. Training Records
  addItem(19, 'Training Records & Attendance', 'Training', 2, false,
    ds.trainingRecordsMaintained, false,
    ds.trainingRecordsMaintained ? 'Training records are maintained.' : 'Training records not maintained.',
    ds.trainingRecordsMaintained ? undefined : 'Implement training record system with attendance tracking.');

  // 20. Independent Audit
  addItem(20, 'Independent Audit / Internal Review', 'Audit', 3, false,
    ds.independentAuditConducted, false,
    ds.independentAuditConducted ? `Independent audit conducted. Last audit: ${ds.lastAuditDate ?? 'date not recorded'}.` : 'No independent audit conducted in the last 12 months.',
    ds.independentAuditConducted ? undefined : 'Schedule independent AML audit per Cabinet Res 134/2025 Art.19.');

  // 21. PF Risk Assessment
  addItem(21, 'PF Risk Assessment', 'PF', 3, false,
    ds.pfRiskAssessmentCurrent, false,
    ds.pfRiskAssessmentCurrent ? 'PF risk assessment is current.' : 'PF risk assessment is outdated or missing.',
    ds.pfRiskAssessmentCurrent ? undefined : 'Conduct PF risk assessment per Cabinet Res 156/2025.');

  // 22. Dual-Use Screening
  addItem(22, 'Dual-Use Goods Screening', 'PF', 3, false,
    ds.dualUseScreeningActive, false,
    ds.dualUseScreeningActive ? 'Dual-use goods screening is active.' : 'Dual-use goods screening not implemented.',
    ds.dualUseScreeningActive ? undefined : 'Implement dual-use/strategic goods screening per Cabinet Res 156/2025.');

  // 23. DPMS Quarterly Reporting
  addItem(23, 'DPMS Quarterly Reporting', 'Reporting', 3, false,
    ds.dpmsQuarterlyReportSubmitted, false,
    ds.dpmsQuarterlyReportSubmitted ? 'DPMS quarterly report submitted.' : 'DPMS quarterly report NOT submitted.',
    ds.dpmsQuarterlyReportSubmitted ? undefined : 'Submit DPMS quarterly report per MoE Circular 08/AML/2021.');

  // 24. Supply Chain DD
  addItem(24, 'Supply Chain Due Diligence', 'Supply Chain', 3, false,
    ds.supplyChainDDDocumented, false,
    ds.supplyChainDDDocumented ? 'Supply chain DD is documented per LBMA RGG v9.' : 'Supply chain DD not documented.',
    ds.supplyChainDDDocumented ? undefined : 'Document supply chain DD per LBMA RGG v9 and UAE MoE RSG Framework.');

  // 25. Tipping-Off Controls
  addItem(25, 'Tipping-Off Controls', 'Controls', 4, true,
    ds.tippingOffControlsActive, false,
    ds.tippingOffControlsActive ? 'Tipping-off controls are active (FDL Art.29).' : 'Tipping-off controls NOT active.',
    ds.tippingOffControlsActive ? undefined : 'CRITICAL: Implement tipping-off controls per FDL Art.29. Staff training required.');
    ...
    inputSchema: {
      ...
      properties: {
        ...
        dataSources: {
          ...
          properties: {
            cddCompletedCount: { type: 'number', description: 'Number of customers with completed CDD' },
            ...
            allSanctionsListsActive: { type: 'boolean', description: 'All 6 lists (UN, OFAC, EU, UK, UAE, EOCN) active' },
          ...
          required: [
            'cddCompletedCount', 'totalActiveCustomers', 'cddOverdueRenewals',
            'goAMLRegistered', 'strsFiledCount', 'strsFiledOnTime',
            'ctrsFiledCount', 'ctrsFiledOnTime', 'allSanctionsListsActive',
            'screeningsPerformed', 'tfsFreezeCapabilityActive',
            'complianceOfficerAppointed', 'coNotifiedToMoE',
            'riskAssessmentCurrent', 'pfRiskAssessmentCurrent',
            'staffTrained', 'totalStaffRequiringTraining',
            'trainingRecordsMaintained', 'independentAuditConducted',
            'recordRetentionCompliant', 'supplyChainDDDocumented',
            'tippingOffControlsActive', 'dualUseScreeningActive',
            'dpmsQuarterlyReportSubmitted', 'uboRegisterCurrent',
            'pepScreeningActive', 'eddAppliedToHighRisk',
          ],
        },
      },
