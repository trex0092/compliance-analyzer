...
export function runPFScreening(
  ...
  if (input.uboSanctionsMatch) {
    alerts.push({
      ...
      mandatoryAction: "Freeze assets without delay. Report to EOCN within 24 hours. File CNMR within 5 business days.",
    });
  }
  ...
  if (input.complexOwnershipStructure) {
    alerts.push({
      ...
      mandatoryAction: "Apply EDD. Reduce UBO threshold from 25% to 10%. Obtain senior management approval.",
    });
  }
  ...
  if (input.customerIsPEP && input.highRiskJurisdiction) {
    alerts.push({
      ...
      mandatoryAction: "Apply EDD. Senior management approval required. Enhanced ongoing monitoring.",
    });
  }
  ...
  if (input.customerDualUseTrading) {
    alerts.push({
      ...
      mandatoryAction: "Verify trading licence for controlled goods. Screen against UAE Strategic Goods Control Lists.",
    });
  }
  ...
  if (input.neighbouringCountryRouting) {
    alerts.push({
      ...
      mandatoryAction: "Investigate routing rationale. Enhanced DD on all intermediaries. Consider STR filing.",
    });
  }
  ...
  if (input.nonFaceToFaceOnboarding && input.highRiskJurisdiction) {
    alerts.push({
      ...
      mandatoryAction: "Apply enhanced identity verification. Collect additional documentation from independent sources.",
    });
  }
  ...
  if (input.inconsistentTransactionProfile) {
    alerts.push({
      ...
      mandatoryAction: "Investigate discrepancy. Obtain source of funds documentation. Consider STR filing.",
    });
  }
  ...
  if (
    ...
    ["defence", "military", "aerospace", "nuclear", "chemical", "biotech"].some(
      (s) => (input.customerSector ?? "").toLowerCase().includes(s)
    )
  ) {
    alerts.push({
      ...
      mandatoryAction: "Apply EDD. Verify customer's business activities are legitimate. Senior management approval required.",
    });
  }
