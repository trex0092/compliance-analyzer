...
| hipaapotamus | `vendor/hipaapotamus` | HIPAA Safe Harbor PII/PHI anonymization library (§164.514(b)(2)) — deterministic SHA-256 aliases, 18-identifier framework, content scanning, nested-object recursion. TypeScript, zero dependencies, MIT. Author: Alex Newman (thedotmack). | **Direct in-scope integration candidate.** Applies to four UAE compliance surfaces: (1) STR narrative scrubbing before any external API call (FDL Art.29 no-tipping-off) — the deterministic alias approach maintains referential integrity across STR/SAR/CNMR cross-references; (2) KYC document PII redaction in logs and audit exports (FDL Art.24 10-year retention without over-retention of raw identifiers); (3) test fixture anonymization so CI never runs against live customer data; (4) UAE identifier extension — adapt the 18-field pattern to Emirates ID, trade licence numbers, UAE passport MRZ, and AED account numbers not in HIPAA scope. Pairs with `src/services/weaponizedBrain.ts` (add as a pre-logging redaction step) and the `goAML` XML exporter (scrub before submission). The `hashSalt` config maps to our existing `SECRETS_SCAN_OMIT_PATHS` pattern. Do NOT use HIPAA vocabulary in UAE-facing MLRO output — adapt field names to UAE PII taxonomy before any user-visible surface. |
| antipattern-czar | `vendor/antipattern-czar` | TypeScript error handling anti-pattern detector — finds `EMPTY_CATCH`, `NO_LOGGING_IN_CATCH`, `LARGE_TRY_BLOCK`, `GENERIC_CATCH`, `PROMISE_EMPTY_CATCH`, `CATCH_AND_CONTINUE_CRITICAL_PATH`, and 4 more patterns. Has a Claude Code `/anti-pattern-czar` command. MIT, requires Bun. | **Direct production utility.** Silent failures on compliance-critical code are audit trail gaps under FDL Art.24. Run `bunx antipattern-czar --config .antipatternrc.json` with `criticalPaths` set to `["netlify/functions/", "src/services/", "src/agents/"]` to enforce catch-or-throw on every sanctions match, STR filing, and CDD escalation path. The approved-override comment pattern (`// [ANTI-PATTERN IGNORED]: ...`) is the right way to document intentional suppression for tight loops — never use a bare empty catch. Install the `/anti-pattern-czar` Claude Code command by copying `vendor/antipattern-czar/.claude/commands/anti-pattern-czar.md` to `.claude/commands/`. Add to the quality gate fan-out (§3) alongside `vitest`, `tsc`, and `eslint`. |
| sequential-thinking-skill | `vendor/sequential-thinking-skill` | Claude Code skill that replicates Sequential Thinking MCP server without requiring a running server process. Disk-persisted `.think_state.json` append-only thought history, revision support, branching, and dynamic depth. TypeScript + tsx, MIT. | **In-scope reasoning augmentation.** Use for complex compliance decisions that must be auditable step-by-step: EDD branching (layering vs. legitimate structuring), sanctions match confidence escalation tree (FDL Art.20 + Cabinet Res 74/2020), and pre-filing STR narrative construction. The disk-persisted state survives `/clear` and session restarts — each thought entry becomes a natural audit trail node. Complements `vendor/tree-of-thought-llm` (BFS/DFS algorithm) and `vendor/dspy` (compiled pipelines): use this skill for interactive reasoning during an MLRO session, ToT for batch EDD dry-runs, DSPy for production pipeline hardening. Install via `npx skills add https://github.com/thedotmack/sequential-thinking-skill --skill sequential-thinking`. |
| notch | `vendor/notch` | Schema-less CLI state machine for LLM agents — append-only JSON log, dynamic field definition via `--field key=value`, type-auto-parsing, `--group-by` views, multi-file support. TypeScript + Bun, MIT. | **Lightweight compliance workflow state tracker.** Fits multi-step compliance pipelines where agent state must survive across tool invocations without a database: CDD/EDD stage progression, four-eyes approval checkpoints, CNMR 5-day countdown steps, and onboarding workflow position. Simpler than `vendor/mcp-shrimp-task-manager` (no MCP server required) and complementary to `vendor/claude-mem` (notch tracks workflow position; claude-mem tracks observations). Use `--state-file` for concurrent workflows (e.g. separate state files per active sanctions investigation). The append-only model means every state transition is auditable — maps directly to the FDL Art.24 audit trail requirement. |
| mcp-client-cli | `vendor/mcp-client-cli` | CLI for any MCP server — dynamic tool discovery via `tools/list`, auto-generated CLI commands from schemas, clean output for piping to `jq`. Works with ChromaDB, GitHub, filesystem, PostgreSQL, and any custom server. TypeScript, MIT. | **Dev and CI utility.** Use to test our `code-review-graph` MCP tools from shell scripts without writing custom client code: `mcp-cli --use code-review-graph get_minimal_context --task "..."`. Fits the quality gate fan-out (§3) for automated impact-radius checks before merging compliance-logic PRs. Also useful for smoke-testing new MCP endpoints during `netlify dev` sessions. Not a production runtime dependency — install globally for dev use: `npm install -g @thedotmack/mcp-client-cli`. |
...
| `thedotmack/rad-mem` | 2026-04-27 | **AGPL-3.0 licence blocker.** Technically superior to `vendor/claude-mem` (progressive disclosure memory retrieval, FTS5 full-text search, PM2-managed worker, web viewer at `:38888`). The "Endless Mode" token compression (~20x more tool uses) is directly useful for long MLRO sessions. **Blocker:** AGPL-3.0 is copyleft and the "network use as distribution" clause means any deployment of a service that links against it also falls under AGPL — incompatible with our proprietary Netlify functions. **Next step:** either (a) request author (Alex Newman / thedotmack) relicense under MIT/Apache-2, or (b) implement the progressive-disclosure retrieval and Endless Mode patterns natively in `vendor/claude-mem` without copying AGPL code. Do not vendor or import under current licence. |
