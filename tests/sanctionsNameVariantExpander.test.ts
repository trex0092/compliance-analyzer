...
describe('transliterateCyrillic', () => {
  it('maps Russian Vladimir to Latin', () => {
    expect(transliterateCyrillic('Владимир')).toBe('vladimir');
  });
  it('maps Putin', () => {
    expect(transliterateCyrillic('Путин')).toBe('putin');
  });
  it('preserves non-Cyrillic characters unchanged', () => {
    expect(transliterateCyrillic('John')).toBe('John');
  });
  it('strips hard + soft signs', () => {
    expect(transliterateCyrillic('Объект')).toBe('obekt');
  });
  it('handles mixed-script input', () => {
    expect(transliterateCyrillic('Иван Smith')).toBe('ivan Smith');
  });
});
...
describe('transliterateGreek', () => {
  it('maps Greek Α to a', () => {
    expect(transliterateGreek('Αθηνα')).toBe('athina');
  });
  it('handles digraphs (theta, chi, psi)', () => {
    expect(transliterateGreek('Θεοδωρος')).toBe('theodoros');
    expect(transliterateGreek('Χαρης')).toBe('charis');
    expect(transliterateGreek('Ψυχη')).toBe('psychi');
  });
  it('preserves non-Greek characters unchanged', () => {
    expect(transliterateGreek('Plato')).toBe('Plato');
  });
});

// ---------------------------------------------------------------------------
// normalize integrates Cyrillic + Greek
// ---------------------------------------------------------------------------

describe('normalize cross-script', () => {
  it('normalises a Cyrillic name to lowercase Latin', () => {
    expect(normalize('Владимир Путин')).toBe('vladimir putin');
  });
  it('normalises a Greek name to lowercase Latin', () => {
    expect(normalize('Αθηνα')).toBe('athina');
  });
  it('survives diacritic strip after transliteration', () => {
    expect(normalize('Έλληνας')).toBe('ellinas');
  });
});
...
describe('applyArabicLatinRules', () => {
  ...
  it('collapses mohd + mhd to mohamed', () => {
    expect(applyArabicLatinRules('mohd abdullah')).toContain('mohamed abdullah');
  });
...
describe('matchesWithVariants', () => {
  ...
  it('matches through a transliteration variant', () => {
    ...
    const result = matchesWithVariants('Mohammed', 'mohamed', exactSimilarity);
    expect(result.matched).toBe(true);
    expect(result.bestVariant).toBe('mohamed');
  });

  it('does not match unrelated names', () => {
    const result = matchesWithVariants('John Smith', 'Mohammed Al-Sabah', exactSimilarity);
