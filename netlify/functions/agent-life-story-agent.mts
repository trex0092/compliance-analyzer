/**
 * Life-Story Synthesiser endpoint — thin stub that accepts input, resolves the
 * Asana module board, writes an audit row, and returns the agent
 * spec + input + placeholder output.
 *
 * POST /api/agents/life-story-agent
 * Required inputs: subjectCode
 * Output shape: life-story-markdown
 * Asana target: screening_and_watchlist
 *
 * Security: Bearer HAWKEYE_BRAIN_TOKEN + rate-limit 20/15min.
 * Regulatory: Cabinet Res 134/2025 Art.7-10 · FATF Rec 10 · FDL Art.24
 *
...
export default async (req: Request, context: Context): Promise<Response> => {
  ...
  const rl = await checkRateLimit(req, { max: 20, clientIp: context.ip, namespace: 'agent-life-story-agent' });
  if (rl) return rl;
  ...
  const asanaBoard = resolveAsanaProjectGid('screening_and_watchlist');
  const result = {
    ok: true,
    agentId: 'life-story-agent',
    name: 'Life-Story Synthesiser',
    ranAt,
    input: body,
    output: {
      status: 'scaffolded',
      note: 'Life-Story Synthesiser execution endpoint live — agent logic implementation queued in follow-on commit. Today this endpoint is gated, audited, and routes to the correct Asana board.',
    },
    asanaBoard,
    regulatoryBasis: 'Cabinet Res 134/2025 Art.7-10 · FATF Rec 10 · FDL Art.24',
    ranBy: auth.username ?? auth.userId ?? null,
  };

  try {
    const store = getStore({ name: 'life-story-audit', consistency: 'strong' });
...
export const config: Config = {
  path: '/api/agents/life-story-agent',
  method: ['POST', 'OPTIONS'],
};
