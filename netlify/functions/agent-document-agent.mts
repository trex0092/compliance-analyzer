/**
 * Document Agent endpoint — thin stub that accepts input, resolves the
 * Asana module board, writes an audit row, and returns the agent
 * spec + input + placeholder output.
 *
 * POST /api/agents/document-agent
 * Required inputs: documentBlobKey,subjectCode
 * Output shape: structured-fields
 * Asana target: onboarding_workbench
 *
 * Security: Bearer HAWKEYE_BRAIN_TOKEN + rate-limit 20/15min.
 * Regulatory: Cabinet Res 134/2025 Art.7-10 · FDL No.10/2025 Art.24 · UAE PDPL Art.6(1)(c)
 *
...
export default async (req: Request, context: Context): Promise<Response> => {
  ...
  const rl = await checkRateLimit(req, { max: 20, clientIp: context.ip, namespace: 'agent-document-agent' });
  if (rl) return rl;
  ...
  const asanaBoard = resolveAsanaProjectGid('onboarding_workbench');
  const result = {
    ok: true,
    ...
    output: {
      status: 'scaffolded',
      note: 'Document Agent execution endpoint live — agent logic implementation queued in follow-on commit. Today this endpoint is gated, audited, and routes to the correct Asana board.',
    },
    asanaBoard,
    regulatoryBasis: 'Cabinet Res 134/2025 Art.7-10 · FDL No.10/2025 Art.24 · UAE PDPL Art.6(1)(c)',
    ranBy: auth.username ?? auth.userId ?? null,
  };

  try {
    const store = getStore({ name: 'document-agent-audit', consistency: 'strong' });
...
export const config: Config = {
  path: '/api/agents/document-agent',
  method: ['POST', 'OPTIONS'],
};
