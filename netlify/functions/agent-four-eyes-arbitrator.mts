/**
 * Four-Eyes Arbitrator endpoint — thin stub that accepts input, resolves the
 * Asana module board, writes an audit row, and returns the agent
 * spec + input + placeholder output.
 *
 * POST /api/agents/four-eyes-arbitrator
 * Required inputs: eventId,firstReviewerName
 * Output shape: second-approver-brief
 * Asana target: four_eyes_queue
 *
 * Security: Bearer HAWKEYE_BRAIN_TOKEN + rate-limit 20/15min.
 * Regulatory: FDL No.10/2025 Art.20-21 · Cabinet Res 134/2025 Art.19 · EU AI Act Art.14
 *
...
export default async (req: Request, context: Context): Promise<Response> => {
  ...
  const rl = await checkRateLimit(req, { max: 20, clientIp: context.ip, namespace: 'agent-four-eyes-arbitrator' });
  if (rl) return rl;
  ...
  const asanaBoard = resolveAsanaProjectGid('four_eyes_queue');
  const result = {
    ok: true,
    agentId: 'four-eyes-arbitrator',
    name: 'Four-Eyes Arbitrator',
    ranAt,
    input: body,
    output: {
      status: 'scaffolded',
      note: 'Four-Eyes Arbitrator execution endpoint live — agent logic implementation queued in follow-on commit. Today this endpoint is gated, audited, and routes to the correct Asana board.',
    },
    asanaBoard,
    regulatoryBasis: 'FDL No.10/2025 Art.20-21 · Cabinet Res 134/2025 Art.19 · EU AI Act Art.14',
    ranBy: auth.username ?? auth.userId ?? null,
  };

  try {
    const store = getStore({ name: 'four-eyes-arbitrator-audit', consistency: 'strong' });
...
export const config: Config = {
  path: '/api/agents/four-eyes-arbitrator',
  method: ['POST', 'OPTIONS'],
};
