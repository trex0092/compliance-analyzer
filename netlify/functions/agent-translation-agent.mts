/**
 * Translation Agent endpoint — thin stub that accepts input, resolves the
 * Asana module board, writes an audit row, and returns the agent
 * spec + input + placeholder output.
 *
 * POST /api/agents/translation-agent
 * Required inputs: sourceText
 * Output shape: translated-text
 * Asana target: screening_and_watchlist
 *
 * Security: Bearer HAWKEYE_BRAIN_TOKEN + rate-limit 20/15min.
 * Regulatory: FATF Rec 10 · FDL No.10/2025 Art.29 · UAE PDPL Art.6(1)(c)
 *
...
export default async (req: Request, context: Context): Promise<Response> => {
  ...
  const rl = await checkRateLimit(req, { max: 20, clientIp: context.ip, namespace: 'agent-translation-agent' });
  if (rl) return rl;
  ...
  const result = {
    ...
    output: {
      status: 'scaffolded',
      note: 'Translation Agent execution endpoint live — agent logic implementation queued in follow-on commit. Today this endpoint is gated, audited, and routes to the correct Asana board.',
    },
    asanaBoard,
    regulatoryBasis: 'FATF Rec 10 · FDL No.10/2025 Art.29 · UAE PDPL Art.6(1)(c)',
    ranBy: auth.username ?? auth.userId ?? null,
  };

  try {
    const store = getStore({ name: 'translation-agent-audit', consistency: 'strong' });
...
export const config: Config = {
  path: '/api/agents/translation-agent',
  method: ['POST', 'OPTIONS'],
};
