/**
 * Report Generator Module — Hawkeye Sterling V2 v2.3
...
.score-card{flex:1;text-align:center;padding:16px 8px;border-radius:3px;color:white;font-size:11px}
...
.finding{border:1px solid #eee;border-radius:3px;padding:16px;margin:12px 0;border-left:4px solid #ccc}
...
.watermark{float:right;color:#bbb;font-size:9px;letter-spacing:2px}
    ...
    html += `<div class="header">
      ...
      <div class="meta"><strong>Company:</strong> ${esc(data.company || 'N/A')} &nbsp;|&nbsp; <strong>Date:</strong> ${new Date().toLocaleDateString('en-GB')} &nbsp;|&nbsp; <strong>Generated by:</strong> Hawkeye Sterling V2 v2.5</div>
    </div>
    <div class="content">`;
    ...
    // Summary
    if (data.summary) {
      html += `<h2>Executive Summary</h2><p style="line-height:1.6">${esc(data.summary)}</p>`;
    }
    ...
    if (data.findings && data.findings.length) {
      ...
      data.findings.forEach((f, i) => {
        ...
        html += `<div class="finding ${esc(sev)}">
          <p><span class="badge" style="${style}">${esc(sev)}</span> &nbsp;<strong style="font-size:14px">${esc(f.title || 'Finding ' + (i + 1))}</strong></p>
          ${f.body ? `<p>${esc(f.body)}</p>` : ''}
          ${f.regulatory_ref ? `<p class="ref">Reference: ${esc(f.regulatory_ref)}</p>` : ''}
          ${f.recommendation ? `<div class="rec"><strong>Recommendation:</strong> ${esc(f.recommendation)}</div>` : ''}
        </div>`;
      });
    }
    ...
    if (data.entityRisks) {
      ...
      data.entityRisks.forEach(r => {
        ...
        html += `<tr><td>${esc(r.entity)}</td><td style="color:${color};font-weight:bold">${r.score}/100</td><td>${esc(r.rating)}</td></tr>`;
      });
      html += `</table>`;
    }
    ...
    if (data.incidents && data.incidents.length) {
      ...
      data.incidents.forEach(inc => {
        html += `<tr><td>${esc(inc.date || 'N/A')}</td><td><strong>${esc(inc.title)}</strong></td><td>${esc(inc.status)}</td><td>${esc(inc.description || '')}</td></tr>`;
      });
      html += `</table>`;
    }

    html += `</div>`;
    html += `<div class="footer"><span class="watermark">CONFIDENTIAL — AUDIT COPY</span>${template.icon} ${template.name} | Hawkeye Sterling V2 v2.5 | Generated ${new Date().toISOString()}</div>`;
    html += `</body></html>`;
...
<div class="card">
  ...
  function generateFromUI() {
    ...
    const defaultSummary = 'This report has been prepared in accordance with the applicable regulatory requirements of the UAE Central Bank, '
      + 'the Financial Action Task Force (FATF) recommendations, and the relevant provisions of Federal Decree-Law No.10 of 2025 on Anti-Money Laundering '
      + 'and Combating the Financing of Terrorism and Proliferation Financing. '
  ...
  function exportCircularsCSV() {
    const list=getCirculars(); if(!list.length){toast('No circulars','error');return;}
    function csvSafe(v){var s=String(v||'').replace(/"/g,'""');if(/^[=+\-@\t\r]/.test(s))s="'"+s;return'"'+s+'"';}
    const csv=[['Reference','Subject','Month','Date'],...list.map(c=>[c.ref,c.subject,c.month,c.date])].map(r=>r.map(csvSafe).join(',')).join('\n');
  ...
  function exportMeetingsDOCX() {
    ...
    list.forEach((m,i)=>{html+='<tr><td>'+(i+1)+'</td><td>'+esc(m.ref)+'</td><td>'+esc(m.month||'')+'</td><td>'+esc(m.date||'')+'</td></tr>';});
    html += '</table>' + (window.wordDocFooter ? window.wordDocFooter() : '</body></html>');
  ...
  function exportMeetingsCSV() {
    const list=getMeetings(); if(!list.length){toast('No meeting minutes','error');return;}
    function csvSafeM(v){var s=String(v||'').replace(/"/g,'""');if(/^[=+\-@\t\r]/.test(s))s="'"+s;return'"'+s+'"';}
    const csv=[['Subject Reference','Month','Date'],...list.map(m=>[m.ref,m.month,m.date])].map(r=>r.map(csvSafeM).join(',')).join('\n');
